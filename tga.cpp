...
std_fread(RLEstate *rleInfo, void *buf, size_t datasize, size_t nelems, FILE *fp)
{
  if (verbose > 1) {
    ...
    printf("TGA: std_fread %ld (total %d)\n",
...
rle_fread(RLEstate *rleInfo, void *vbuf, size_t datasize, size_t nelems, FILE *fp)
{
  ...
  while (j < buflen) {
    ...
    if (count & RLE_PACKETSIZE) {
      ...
      if (fread(p, datasize, 1, fp) != 1) {
        if (verbose) {
          printf("TGA: EOF while reading %d/%ld element RLE packet\n",
            bytes, datasize);
        }
        return j / datasize;
      }
      ...
      if (fread(p, bytes, 1, fp) != 1) {
        if (verbose) {
          printf("TGA: EOF while reading %d/%ld element raw packet\n",
            bytes, datasize);
        }
        return j / datasize;
      }
  ...
  if (verbose > 1) {
    printf("TGA: rle_fread %ld/%d (total %d)\n",
